...
int main(int argc, char** argv) {
    ...
    if (!mapping) {
        GpConPrintf("找不到状态段 %ls\n", GPRDR2_NAME);
        GpConPrintf("说明游戏内组件（gpxinput_rdr2.asi）没在跑：\n");
        GpConPrintf("  · 文件是不是还在游戏根目录（别改成 .off）\n");
        GpConPrintf("  · ScriptHookRDR2.log 里有没有 'Registering script gpxinput_rdr2.asi'\n");
        GpConPrintf("  · 游戏内的 gpxinput_rdr2.log 里写了什么\n");
        return 2;
    }
    ...
    if (!st) {
        GpConPrintf("映射失败 (err=%lu)\n", GetLastError());
        return 2;
    }

    GpConPrintf("状态段已连接: %ls\n", GPRDR2_NAME);
    GpConPrintf("magic=0x%08X version=%u (期望 0x%08X / %u)\n",
           st->magic, st->version, GPRDR2_MAGIC, GPRDR2_VERSION);
    if (st->magic != GPRDR2_MAGIC || st->version != GPRDR2_VERSION) {
        GpConPrintf("段格式对不上 —— .asi 和这个工具不是同一版，重新编译一次\n");
        return 2;
    }
    ...
    for (;;) {
        ...
        if (!once) GpConPrintf("\x1b[2J\x1b[H");   

        DWORD age = (DWORD)(GetTickCount() - s.tickMs);
        GpConPrintf("========== gpxinput 游戏状态 ==========\n");
        if (s.frame == 0 || s.tickMs == 0) {
            

            GpConPrintf("  心跳     组件已建段，但还没有发布任何一帧\n");
            GpConPrintf("           多半是游戏还在主菜单/加载中（玩家的 ped 还是 0）；\n");
            GpConPrintf("           进到世界里会自动开始。若一直如此，就看游戏目录的\n");
            GpConPrintf("           gpxinput_rdr2.log 里有没有「脚本线程已启动」那一行。\n");
